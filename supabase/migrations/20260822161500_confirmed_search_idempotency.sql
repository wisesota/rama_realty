-- A confirmed brief is the only operation allowed to create a search run.
-- The buyer-scoped key makes double-confirm, reconnect, and retry safe.

alter table public.search_runs
  add column if not exists confirmation_key text;

alter table public.search_runs
  drop constraint if exists search_runs_confirmation_key_format;
alter table public.search_runs
  add constraint search_runs_confirmation_key_format
  check (confirmation_key is null or char_length(confirmation_key) between 16 and 128);

create unique index if not exists search_runs_buyer_confirmation_key_idx
  on public.search_runs (buyer_session_id, confirmation_key)
  where buyer_session_id is not null and confirmation_key is not null;

drop function if exists public.persist_buyer_search(text, text, text, jsonb, jsonb, uuid, text, integer);

create function public.persist_buyer_search(
  p_token_hash text,
  p_source text,
  p_raw_brief text,
