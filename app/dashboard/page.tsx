...
export default async function DashboardPage() {
  ...
  return <DashboardShell staff={staff} email={email} active="overview">
    ...
    <section className="ops-action-queue" aria-label="Action queue">
      <Link href="/dashboard/inventory?health=attention"><span className="ops-action-icon" data-tone="sand"><RefreshCw aria-hidden="true" /></span><span><small>Source health</small><strong>{stale} record{stale === 1 ? "" : "s"} need refresh</strong><em>{stale ? "Update evidence before publication expires." : "No source freshness blockers detected."}</em></span><ArrowRight aria-hidden="true" /></Link>
      <Link href="/dashboard/inventory?status=in_review"><span className="ops-action-icon" data-tone="blue"><Building2 aria-hidden="true" /></span><span><small>Publication review</small><strong>{inReview} record{inReview === 1 ? "" : "s"} waiting</strong><em>{inReview ? "Preview exactly what Rama can disclose." : "Nothing is waiting for an editor."}</em></span><ArrowRight aria-hidden="true" /></Link>
      <Link href="/dashboard/inquiries"><span className="ops-action-icon" data-tone="green"><MessageSquareText aria-hidden="true" /></span><span><small>Buyer follow-up</small><strong>{openInquiries} conversation{openInquiries === 1 ? "" : "s"} open</strong><em>{openInquiries ? "Continue with the complete buyer context." : "No advisor follow-ups are outstanding."}</em></span><ArrowRight aria-hidden="true" /></Link>
    </section>

    <section className="ops-readiness" aria-labelledby="readiness-title"><div><p>Catalog readiness</p><h2 id="readiness-title">{published} of {properties.length} residences available to Rama</h2><span>{properties.length ? `${inReview} in review · ${stale} source blocker${stale === 1 ? "" : "s"}` : "Add the first governed residence to begin serving live inventory."}</span></div><div className="ops-readiness-score"><strong>{readiness}%</strong><span>buyer-visible</span></div><div className="ops-readiness-track" aria-hidden="true"><span style={{ width: `${readiness}%` }} /></div><Link href="/dashboard/inventory">Open inventory <ArrowRight aria-hidden="true" /></Link></section>

    <div className="ops-brief-grid">
      <section className="ops-ledger-panel"><SectionHeader eyebrow="Buyer conversations" title="Advisor queue" meta={`${openInquiries} requiring follow-up`} action={<Link href="/dashboard/inquiries">View all <ArrowRight aria-hidden="true" /></Link>} />{inquiries.length ? <div className="ops-brief-list">{inquiries.map((inquiry) => { const property = Array.isArray(inquiry.properties) ? inquiry.properties[0] : inquiry.properties; return <Link key={inquiry.id} href="/dashboard/inquiries"><span className="ops-avatar" aria-hidden="true">{inquiry.full_name.slice(0,1).toUpperCase()}</span><span><strong>{inquiry.full_name}</strong><small>{property?.name ?? "Property context unavailable"}</small></span><span><i className="ops-status" data-status={inquiry.status}>{inquiry.status.replace("_", " ")}</i><small>{new Intl.DateTimeFormat("en-AE", { dateStyle: "medium" }).format(new Date(inquiry.created_at))}</small></span></Link>; })}</div> : <div className="ops-panel-empty"><MessageSquareText aria-hidden="true" /><strong>No advisor handoffs yet.</strong><span>Consented buyer conversations will arrive with their property and search context attached.</span><Link href="/" target="_blank" rel="noopener noreferrer">Test the buyer experience <ArrowRight aria-hidden="true" /><span className="sr-only"> in a new tab</span></Link></div>}</section>
