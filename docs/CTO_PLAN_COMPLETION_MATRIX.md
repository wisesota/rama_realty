...
| Supabase leaked-password protection | Intentionally not used | Enabling `password_hibp_enabled` returned HTTP 402 on the Free organization. The CTO decision is to remain on Supabase Free, so this paid control is removed from staging and production gates. Existing password validation, rate limiting, native Supabase authentication, and the staff MFA/step-up gate remain required. |
